  ...
  try {
    switch (name) {
      case "gatelane_health": {
        return {
          content: [{ type: "text", text: JSON.stringify({ status: "ok", service: "gatelane", version: "0.1.0", cloudMode: isCloudMode() }) }],
        };
      }
...
export function createMcp(port: number = 3052): void {
  ...
  app.post("/mcp", async (req, res) => {
    ...
    if (method === "tools/list") {
      ...
      res.json({ status: "ok", service: "gatelane-mcp", version: "0.1.0" });
    } else {
      res.json({
        jsonrpc: "2.0",
        id: params?.id || 1,
